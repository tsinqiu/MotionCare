...
async function ensureFeedbackTable() {
  if (await tableExists('AiCoachFeedback')) {
    return;
  }
...
async function ensureMorningReadinessTable() {
  if (await tableExists('MorningReadinessFeedback')) {
    return;
  }
...
async function getHealth() {
  ...
  const rulesAvailable = Boolean(config.ai.fallbackRules);
  const coach = await coachMlService.getHealth();
  return {
    status: deepseekConfigured || rulesAvailable ? 'ok' : 'unavailable',
    provider: 'deepseek',
