...
test('Capacitor config targets a MotionCare Android app from Vite dist', () => {
  ...
  assert.match(capacitorConfig, /hostname:\s*['"]127\.0\.0\.1['"]/)
  assert.match(capacitorConfig, /androidScheme:\s*['"]http['"]/)
})

test('Android WebView uses a local HTTP origin and allows current HTTP assets', () => {
