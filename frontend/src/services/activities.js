...
export function normalizeTrackPoint(row = {}) {
  return {
    ...
    accuracy_m: toNumber(firstDefined(row.accuracy_m, row.accuracyM)),
    bearing_deg: toNumber(firstDefined(row.bearing_deg, row.bearingDeg)),
    provider: firstDefined(row.provider, row.locationProvider),
    is_accepted: firstDefined(row.is_accepted, row.isAccepted),
    reject_reason: firstDefined(row.reject_reason, row.rejectReason),
