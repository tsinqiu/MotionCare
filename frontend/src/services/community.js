...
export function normalizePost(row = {}) {
  return {
    ...
    activityLocationName: row.activityLocationName || row.activity_location_name || row.locationName || row.location_name || '',
    distanceM: Number(row.distanceM ?? row.distance_m ?? row.total_distance_m ?? 0),
    durationS: Number(row.durationS ?? row.duration_s ?? row.total_timer_time_s ?? 0),
    elevationGainM: Number(row.elevationGainM ?? row.elevation_gain_m ?? row.total_ascent_m ?? 0),
    calories: Number(row.calories ?? row.total_calories ?? 0),
    activityTrainingLoad: Number(row.activityTrainingLoad ?? row.activity_training_load ?? 0),
    weatherCondition: row.weatherCondition || row.weather_condition || '',
    temperatureC: row.temperatureC ?? row.temperature_c ?? null,
